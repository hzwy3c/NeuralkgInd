...
def setup_parser():
    ...
    parser.add_argument('--add_traspose_rels', '-tr', type=bool, default=False, help='whether to append adj matrix list with symmetric relations') # 邻接矩阵 对称关系
    ...
    parser.add_argument('--gnn_agg_type', '-a', type=str, choices=['sum', 'mlp', 'gru'], default='sum', help='what type of aggregation to do in gnn msg passing') # gnn agg strategy
    parser.add_argument('--has_attn', '-attn', type=bool, default=True, help='whether to have attn in model or not') # 
    parser.add_argument("--num_gcn_layers", "-l", type=int, default=3, help="Number of GCN layers") # GNN层数
    parser.add_argument("--rel_emb_dim", "-r_dim", type=int, default=32, help="Relation embedding size") # 关系维度
    parser.add_argument("--attn_rel_emb_dim", "-ar_dim", type=int, default=32, help="Relation embedding size for attention") # 关系注意力维度
    parser.add_argument("--edge_dropout", type=float, default=0.5, help="Dropout rate in edges of the subgraphs") # dropout
    #sampler
    parser.add_argument("--hop", type=int, default=3, help="Enclosing subgraph hop number") # 采样跳数
    parser.add_argument("--enclosing_sub_graph", "-en", type=bool, default=True, help='whether to only consider enclosing subgraph')  # 子图
    parser.add_argument("--num_neg_samples_per_link", "-neg", type=int, default=1, help="Number of negative examples to sample per positive link") # 负采样bs
    #presubgraph
