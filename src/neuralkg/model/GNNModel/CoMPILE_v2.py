...
class CoMPILE_v2(nn.Module):              
    def __init__(self, args):
        ...
        self.final_relation_embeddings = nn.Parameter(torch.randn(self.params.aug_num_rels, self.params.rel_emb_dim))
    ...
    def batch_subgraph(self, subgraph):
        ...
        # source_node = np.array(source_node.cpu()); target_node = np.array(target_node.cpu())   
        source_node = np.array(torch.tensor(source_node, device='cpu')); target_node = np.array(torch.tensor(target_node, device='cpu')) 
